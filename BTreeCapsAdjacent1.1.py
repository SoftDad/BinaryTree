...
#			print(level, "found one on right: ", a_node.item.word)
	...
	def search (self, word, func, level=0, item=[], a_node=none)
		...
		if a_node is None:
			...
			if a_node.item is None:
				...
				if new_item.value < a_node.item.value:
					self.search(a_node.left, level + 1)
				elif: new_item.value > a_node.item.value:
					self.search(a_node.right, level + 1)
...
parse_text (words, btree.add_node)
...
#print()
#print("OK, Here are the nodes, in order:")
