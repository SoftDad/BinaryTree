...
#		print(self.word)
		...
		ln = len(self.word)
		if ln > max_word_len: ln = max_word_len		# sure would like to use lesser_of()
		value_text = ""
		for i in range(ln):
...
##		This stuff is not used, just left it here as I though it might be useful
##		if it ever comes to balancing the tree...
##
...
class BTree:
	...
	def add_item (self, word):
		...
		item  = Item(word)
		print (item.value, item.word)	# left this in just to see the values and words going in
		node = Node(item)
		return self.add_node(node)
	
	def add_node (self, node, cur_node = None):  # Need to go recursive, starting with None => root
		...
		if cur_node is None:
			...
			if cur_node.left is None:
				...
				ret_val = self.add_node(node, cur_node.left)
		elif node.item.value > cur_node.item.value:
			if cur_node.right is None:
				...
				ret_val = self.add_node(node, cur_node.right)
...
# A few misc functions we need to support building an Item.
# Compute a value for a letter
def letter_value(ch):
	""" Interleave upper and lower case, so value of a is that of A + 1, etc """
	lval = ord(ch)
	if lval >= ord("A") and lval <= ord("Z"):		# It's upper case
		ret_val = ((lval - ord("A")) * 2) + base_letter_value
	elif lval >= ord("a") and lval <= ord("z"):		# It's lower case
		ret_val = ((lval - ord("a")) * 2) + base_letter_value + 1
	return ret_val
...
# Here is a long stream of words, which we'll stick into the tree...
...
# Here's the real starting place, i.e. "main()"
# The items (words) will get printed along with their values as they go in,
# so here's a little header for that list:
print("Value              Word")

# Instatiate the tree - we'll be needing it soon...
b_tree = BTree()

# Parse the string, inserting each word item into the tree
i = 0
while i < len(words):
	start = i
...
#	print(f"end is {end}")
	...
	if len(word) > 0:
#		print(len(word), word)
		b_tree.add_item(word)

def print_node(a_node):
	print(a_node.item.value, a_node.count, a_node.item.word)	

# Traverse the tree, printing each node
print()
print("""OK, Here are the nodes,
in order, with values and number of occurances:""")
